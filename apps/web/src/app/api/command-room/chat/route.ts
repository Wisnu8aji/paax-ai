 ...
 * 3 model: Lucent, Arete, dan Noir memakai Mimo v2.5 via opencode-go. Semua memakai 1 API key
...
import { z } from "zod";
import type { GatewayTurnPrepared } from "@paax/schemas";
import {
  type ModelAlias,
...
import { resolveLegacyProfile, type LegacyWorkProfile } from "../work/legacy-profile";
...
function getConfiguredModelSlug(alias: ModelAlias): string {
  const rawProfiles = process.env.PAAX_MODEL_PROFILES_JSON?.trim();
  if (rawProfiles) {
    try {
      const profiles = JSON.parse(rawProfiles) as Record<string, unknown>;
      const profile = profiles[alias];
      if (profile && typeof profile === "object" && "model" in profile) {
        const configured = (profile as { model?: unknown }).model;
        if (typeof configured === "string" && configured.trim()) return configured.trim();
      }
    } catch {
      // Fall back to the typed model catalog when optional JSON is malformed.
    }
  }
  return getModel(alias).apiModel;
}
...
const OPENROUTER_MODEL_SLUG: Record<ModelAlias, string> = {
  lucent: getConfiguredModelSlug("lucent"),
  arete: getConfiguredModelSlug("arete"),
  noir: getConfiguredModelSlug("noir"),
};
 ...
 * Mimo via opencode-go memakai mapping high/xhigh biasa.
 */
...
async function streamOpenRouter(
  ...
  const model = modelSlugOverride ?? OPENROUTER_MODEL_SLUG[modelAlias];
...
// ─── Command Room — opencode-go native ─────────────────────────────────────
...
function buildDeepSeekPayload(
  ...
  effort: ReasoningEffort,
  modelAlias: ModelAlias,
  apiModelOverride?: string,
): Record<string, any> {
  const payload: Record<string, any> = {
    model: apiModelOverride ?? getModel(modelAlias).apiModel,
    messages: withSystemPrompt(messages),
    stream: true,
  };
  if (thinking === "on") {
...
async function streamDeepSeekNative(
  ...
  for (let steerCount = 0; steerCount <= 3; steerCount++) {
    const payload = buildDeepSeekPayload(currentMessages, thinking, effort, modelAlias, apiModelOverride);
...
async function handleWorkPost(
  req: NextRequest,
  requestBody?: unknown,
  options: { prepared?: GatewayTurnPrepared } = {},
): Promise<Response> {
  ...
  const runId = options.prepared?.runId || workRequest.runId?.trim() || `work-run-${crypto.randomUUID()}`;
  const conversationId = workRequest.session.conversationId;
  ...
  const legacyProfile: LegacyWorkProfile | null = options.prepared ? resolveLegacyProfile(options.prepared.profile) : null;
  if (options.prepared && !legacyProfile) {
    return NextResponse.json({ error: "Work handoff belum didukung oleh web transport." }, { status: 503 });
  }
  if (!options.prepared && !["lucent", "arete", "noir"].includes(workRequest.modelAlias)) {
    return NextResponse.json({ error: "Work profile belum didukung oleh legacy web transport." }, { status: 503 });
  }
  const runtimeModelAlias: ModelAlias = legacyProfile?.modelAlias ?? workRequest.modelAlias as ModelAlias;
  const runtimeApiModel = legacyProfile?.apiModel ?? getModel(runtimeModelAlias).apiModel;
  const runtimeOpenRouterModel = legacyProfile?.openRouterModelSlug ?? OPENROUTER_MODEL_SLUG[runtimeModelAlias];
  const resolved = resolveKeyForModel(runtimeModelAlias);
  if (!resolved) {
  ...
  const resolvedThinking = options.prepared?.profile.thinking ?? resolveThinking(runtimeModelAlias, workRequest.thinking);
  const effort = (options.prepared?.profile.selectedEffort ?? workRequest.reasoningEffort) as ReasoningEffort;
    ...
    async start(controller) {
      ...
      try {
        emitter.emit("turn.started", {
          ...
          ...(options.prepared ? { gatewaySessionId: options.prepared.sessionId } : {}),
        });
        ...
        try {
          ...
          const toolResult = resolved.viaOpenRouter
            ? await runOpenRouterWithTools({
                modelSlug: runtimeOpenRouterModel,
                modelAlias: runtimeModelAlias,
        ...
        if (resolved.viaOpenRouter) {
          await streamOpenRouter(runtimeModelAlias, flattenToolHistoryToChatMessages(finalMessages), resolvedThinking, effort, resolved.apiKey, req, sendProviderEvent, runId, conversationId, runtimeOpenRouterModel);
        } else {
          await streamDeepSeekNative(flattenToolHistoryToChatMessages(finalMessages), resolvedThinking, effort, resolved.apiKey, req, sendProviderEvent, runId, conversationId, runtimeModelAlias, runtimeApiModel);
        }
  ...
  const readableStream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Chat v1.5 exposes only general capabilities. Operational
        // Drawing/RAB/Schedule tools remain isolated in handleWorkPost.
        ...
        if (generalTools.length) {
          const toolResult = resolved.viaOpenRouter
            ? await runOpenRouterWithTools({
                ...
                modelAlias,
                apiKey: resolved.apiKey,
                messages: attachmentMessages as ToolChatMessage[],
                context: undefined,
                connectors: [],
            ...
            : await runDeepSeekNativeWithTools({
                apiModel: getModel(modelAlias).apiModel,
                modelAlias,
                baseUrl: getDeepSeekBaseUrl(),
                apiKey: resolved.apiKey,
                messages: attachmentMessages as ToolChatMessage[],
