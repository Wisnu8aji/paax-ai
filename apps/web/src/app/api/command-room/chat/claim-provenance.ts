...
export type ClaimType =
  ...
  | "schedule_value" | "contextual_occurrence" | "unknown";
...
export function buildClaims(input: {
  ...
  if (isContextualPhysicalClaim && candidates.length === 0) {
    // Use the actual matched substring (not a synthetic placeholder) so
    // callers redacting/removing this claim from the response text by
    // matching claim.text against the real string can actually find it.
    candidates.push({ text: contextualMatch[0], value: null, claim_type: "contextual_occurrence" });
  }
