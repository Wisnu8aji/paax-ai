...
const categoryTone: Record<string, 'ok' | 'warn' | 'dng' | 'neutral'> = {
  bahan: 'ok',
  upah: 'warn',
  alat: 'dng',
};
...
function HspBreakdownBody({ data }: { data: HSPBreakdown }) {
  const th: React.CSSProperties = { padding: '9px 12px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text2)', borderBottom: '1px solid var(--border)' };
  const td: React.CSSProperties = { padding: '9px 12px', borderBottom: '1px solid var(--border-soft)' };
  return (
    <div>
      <div style={{ fontSize: 11.5, color: 'var(--text3)', marginBottom: 12 }}>{data.ahsp_code} · per {data.unit}</div>
      <div style={{ overflowX: 'auto' }}>
          ...
          <tbody>
            {data.components.map((c) => (
              <tr key={c.resource_code}>
                <td style={{ ...td, color: 'var(--text)' }}>{c.resource_name} <span style={{ color: 'var(--text3)' }}>[{c.unit}]</span></td>
                <td style={td}><StatusPill tone={categoryTone[c.category] ?? 'neutral'}>{c.category}</StatusPill></td>
