...
export default function CommandRoomPage() {
  ...
  function submitCreateProject() {
    ...
    if (!name) return;
    createFolder(SCOPE, name, newGoal);
    setNewName('');
    setNewGoal('');
    setCreateOpen(false);
    refresh(activeId);
    showNote(`Project "${name}" dibuat.`);
  }
  ...
  const filteredProjects = folders.filter(
    (f) => !projectSearch || f.name.toLowerCase().includes(projectSearch.toLowerCase()),
  );
  const projectConvCount = (folderId: string) => conversations.filter((c) => c.folderId === folderId).length;
  const projectUpdatedAt = (f: ChatFolder) => {
    const inside = conversations.filter((c) => c.folderId === f.id);
    return inside.length > 0 ? inside.map((c) => c.updatedAt).sort().reverse()[0] : f.createdAt;
  };
  const openProject = openProjectId ? folders.find((f) => f.id === openProjectId) ?? null : null;
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 9 }}>
          ...
          <div ref={addToRef} style={{ position: 'relative' }}>
            ...
            >
              <FolderPlus size={13} />
              {active?.folderId ? folders.find((f) => f.id === active.folderId)?.name ?? 'Add to Project' : 'Add to Project'}
            </button>
            {addToOpen && (
              <div className="pax-scale-in" role="menu" style={{ position: 'absolute', bottom: 40, left: 0, width: 230, borderRadius: 13, background: 'var(--cr-panel2)', border: '1px solid var(--cr-border-strong)', boxShadow: '0 18px 44px rgba(0,0,0,0.5)', padding: 5, zIndex: 40, maxHeight: 260, overflowY: 'auto' }}>
                ...
                {folders.map((f) => (
                  <button
                    ...
                    onClick={() => {
                      if (active) {
                        moveConversation(active.id, f.id);
                        refresh(active.id);
                        showNote(`Percakapan masuk ke "${f.name}".`);
                      } else {
                        const conv = createConversation(SCOPE, f.id);
                        refresh(conv.id);
                        showNote(`Chat baru dibuat di "${f.name}".`);
                      }
                      setAddToOpen(false);
                    }}
                    className="pax-cr-hover"
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 9, padding: '9px 10px', borderRadius: 9, border: 'none', background: active?.folderId === f.id ? 'rgba(255,255,255,0.08)' : 'transparent', color: 'var(--cr-text)', fontSize: 12.5, cursor: 'pointer', textAlign: 'left' }}
                  >
  ...
  return (
    <div className="pax-command" style={{ display: 'flex', flex: 1, minHeight: 'calc(100vh - 36px)', borderRadius: '0 28px 28px 0', overflow: 'hidden' }}>
      {/* ══ SIDEBAR ══ */}
      <div
        style={{
          width: 268,
                ...
                <div className="pax-fade">
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
                    ...
                    <button
                      ...
                      className="pax-press"
                      style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 14px', borderRadius: 10, border: 'none', background: '#fff', color: '#1A1915', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}
                    >
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    <button
                      ...
                      className="pax-press"
                      style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 15px', borderRadius: 10, border: 'none', background: '#fff', color: '#1A1915', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}
                    >
                      New project
                    </button>
                  </div>
                  ...
                  <div className="pax-stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14, marginTop: 22 }}>
                    ...
                    {filteredProjects.length === 0 && (
                      <div style={{ gridColumn: '1 / -1', padding: '40px 0', textAlign: 'center', color: 'var(--cr-text3)', fontSize: 13 }}>
                        {folders.length === 0 ? 'Belum ada project. Klik "New project" untuk memulai.' : 'Tidak ada project yang cocok.'}
                      </div>
                    )}
                  </div>
                </>
      ...
      {createOpen && (
        ...
        >
          <div className="pax-scale-in" style={{ width: 520, maxWidth: '100%', background: '#26241F', border: '1px solid var(--cr-border-strong)', borderRadius: 16, padding: '22px 24px', boxShadow: '0 30px 80px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 9, marginTop: 20 }}>
              ...
              <button
                ...
                className="pax-press"
                style={{ padding: '9px 17px', borderRadius: 10, border: 'none', background: '#fff', color: '#1A1915', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', opacity: newName.trim() ? 1 : 0.5, transition: 'opacity .2s var(--ease)' }}
              >
                Create project
              </button>
            </div>
