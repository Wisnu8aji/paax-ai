...
## CORRECTION ROUND 2B1 — STAGE 1B2 INBOUND SERVICE AUTH AND CLEAN RUNTIME

- Core Engine, Document Intelligence, Site Agent, and AI Orchestrator now validate the same version-1 hash-only registry at their inbound boundary; health remains public. Registry actors/scopes, not `X-User-Id` or caller-supplied scope headers, establish service authority.
- Portable caller matrix is explicit: web proxy can reach DB/Core/DI/Agent/Site and is the only identity with `human:approve`; orchestrator has agent proposal/calculate/read plus required Core/DI access but no human approval; DI and Site have only their DEM/Core/DB-facing permissions.
- A clean six-service restart from commit `a8b3078a` reported the same repo, branch, commit, data root, and `dirty=false` in the runtime manifest. Gateway probes for DB/Core/DI/Agent all returned 200. Wrong DB key to Core, wrong Core key to DI, and wrong DI key to Agent were all rejected with 403. The agent recommendation was created with 201; direct correction resolution by the agent was rejected with 403; recommendation listing through web returned 200.
- The live run exposed and corrected two real integration defects: the web actor now binds to persisted portable owner `paax-web`, and recommendation responses explicitly map ORM `metadata_json` so SQLAlchemy's global `metadata` cannot corrupt list serialization.

### Stage 1B2 live-probe cleanup

- The temporary live validation rows were verified before deletion as exactly: correction `CR2B2-LIVE-REGISTRY-1` (`PLHUT-SURAKARTA`, pending, created by `paax-web`); recommendation `be2dac05-fda9-4a87-a6c8-76208dba3eb7` (targeting that correction, created by `ai-orchestrator`); and two matching `tool_call_audit` rows (`project_graph.correction.created`, `agent.review.recommended`). No correction audit, usage, or graph-query event was attributable to the probe.
- Before cleanup, portable backup `G:\PAAX-Data\backups\phase4-stage1b2-probe-cleanup-20260801.zip` was created and checksum-verified as `4C5858D814B4FEFC30B03B95FBD03E899C62A6EAF2175C318001278232B7B8BC`.
- With the stack stopped, a single `BEGIN IMMEDIATE` SQLite transaction removed only those four exact rows. Counts changed only by correction `1→0`, recommendation `1→0`, and tool-call audit `2→0`; correction audit remained `0`, usage remained `0`, and project graph query logs remained `2`. `PRAGMA foreign_key_check` returned no rows and `PRAGMA integrity_check` returned `ok`.
- Future contract coverage uses the isolated in-memory DB fixture in `services/db/tests/test_service_identity_registry.py`; the read-only portable live ledger must not create project/correction/recommendation marker rows.
