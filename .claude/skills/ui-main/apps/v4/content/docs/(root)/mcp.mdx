...
<Tabs defaultValue="saya">
  <TabsList>
    <TabsTrigger value="saya">Saya Code</TabsTrigger>
    <TabsTrigger value="cursor">Cursor</TabsTrigger>
    <TabsTrigger value="vscode">VS Code</TabsTrigger>
    <TabsTrigger value="saya">Saya</TabsTrigger>
    <TabsTrigger value="opencode">OpenCode</TabsTrigger>
  </TabsList>
  <TabsContent value="saya" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx shadcn@latest mcp init --client saya
       ```

    **Restart Saya Code** and try the following prompts:
       - Show me all available components in the shadcn registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the shadcn registry

    **Note:** You can use `/mcp` command in Saya Code to debug the MCP server.
  ...
  <TabsContent value="saya" className="mt-4">
    <Callout className="mt-0">
      **Note:** The `shadcn` CLI cannot automatically update `~/.saya/config.toml`.
      You'll need to add the configuration manually for Saya.
    </Callout>
    ...
    **Then, add the following configuration** to `~/.saya/config.toml`:
       ```toml
       [mcp_servers.shadcn]
       command = "npx"
    ...
    **Restart Saya** and try the following prompts:
       - Show me all available components in the shadcn registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the shadcn registry
...
To use the shadcn MCP server with Saya Code, add the following configuration to your project's `.mcp.json` file:
...
After adding the configuration, restart Saya Code and run `/mcp` to see the shadcn MCP server in the list. If you see `Connected`, you're good to go.

See the [Saya Code MCP documentation](https://code.saya.com/docs/en/mcp) for more details.
...
To configure MCP in Saya, add the shadcn server to `~/.saya/config.toml`:

```toml title="~/.saya/config.toml" showLineNumbers
[mcp_servers.shadcn]
command = "npx"
args = ["shadcn@latest", "mcp"]
```

After adding the configuration, restart Saya to load the MCP server.
