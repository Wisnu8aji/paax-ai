...
> Saya/Saya berikutnya tidak perlu re-analisis dari nol, temuannya
...
- Status implementasi tiap baris di tabel "Disesuaikan" di-update seiring
  fase dikerjakan — cek juga `docs/history/STATE_ARCHIVE_2026-06_2026-07.md` untuk status terkini.
...
|---|---|---|---|
| Aturan Emas | `SAYA.md` §1: AI tak pernah hitung, engine Python yang hitung | INV-01/02 (TXT01 §1): identik, diperluas jadi 13 invariant | Brain **memformalkan**, bukan mengubah |
| Arsitektur 4-lapis | `SAYA.md` §3: Presentasi/Orkestrasi/Persepsi/Engine/Site Agent/Data | brain L0–L3 (TXT03): Web/Orchestrator/document-intelligence(L2A)/core-engine(L2B)/Data(L3) | Peta 1:1, tidak ada tabrakan |
| Monorepo, Zod↔Pydantic 1 sumber kebenaran | `SAYA.md` §4, ADR-0001 | TXT03 §"Data Schema": `@paax/schemas` Zod↔Pydantic | Sama |
...
| Gerbang review branch→PR, tidak auto-merge | `SAYA.md` §9 | Tidak dibahas brain (di luar cakupan) | Tetap berlaku, tidak tersentuh |
...
| Rezim testing | `SAYA.md` §2: nilai acuan manual per fungsi kalkulasi baru | TXT03 §6: T-01–T-08 (property-based geometri, golden-anchor 1 proyek nyata, golden TKG, eval per-skill) | Perlu diperkaya bertahap seiring modul rumus baru | 🟡 rujukan ditambahkan di `SAYA.md` §2 |
...
Prinsip: **vertical slice** (`SAYA.md` §2) — tidak membangun 92 rumus sekaligus,
dan **tidak menyentuh CV/vision** sampai gerbang F0 (data grounding) selesai
sesuai `docs/history/STATE_ARCHIVE_2026-06_2026-07.md`.

| Fase | Isi | Status |
|---|---|---|
| Fase 0 | Salin brain verbatim ke `docs/specs/brain-v4.1/` + dokumen ini + rujukan di STATE/START_HERE/MASTER_PLAN/security/SAYA.md | 🟢 selesai |
