...
> **Catatan penting soal peran AI (2026-07-05).** Spek "brain" itu sendiri
> pada dasarnya adalah desain **lapisan reasoning AI** (arsitektur kognitif
> L0-L9, confidence/triangulasi F-J01-J05, katalog 31 skill) — jadi gap
> analysis di dokumen ini SEBAGIAN BESAR adalah gap "seberapa dalam lapisan
> AI PAAX sudah menginterpretasi", BUKAN gap kalkulasi (rumus §B-G sudah
> matang, lihat §4). Fase X2 (AI-assist klasifikasi/binding, §4 di bawah)
> adalah realisasi konkret pertama dari arah ini: LLM sekarang jadi fallback
> interpretasi di 8 kategori elemen gambar. Ini menguatkan bahwa AI memang
> lapisan pusat produk (`CLAUDE.md` §0.1) — sekaligus brain v4.1 menegaskan
> batasnya tetap sama: AI menginterpretasi, tidak pernah menghitung (INV-01).
...
|---|---|---|---|---|
| Rumus takeoff engine | `services/core-engine/app/takeoff/` + `app/tkg/takeoff.py` + `app/geometry/volume.py` — beton per bentuk (F-B), besi+BBS (F-D), tanah (F-F01-05,07), arsitektur/finishing (F-E), atap/kusen/MEP (F-G04,06-14), dinding beton/kolom/tangga/perancah (F-C07-10) | TXT02: 92 rumus (F-A00–K08) — beton per bentuk, bekisting, besi+BBS, tanah bank/gembur/padat, arsitektur/finishing, atap/kusen/MEP, confidence, QA | **UPDATE 2026-07-05 (lihat baris Fase 3+ di §4): klaim "±5%" di baris ini SUDAH USANG.** Mayoritas rumus §B-G sudah ada & teruji (lihat Fase 1-3+ §4). Sisa gap murni §J/K (confidence/QA formal) + sebagian §H-I (jadwal lanjutan) — bukan lagi "rumus dasar saja" | 🟢 mayoritas rumus §B-G selesai — lihat baris Fase 1/3a/3b/3+ di §4 utk rincian |
...
| Fase 3+ | **KOREKSI 2026-07-05 (audit B0 v1.0): baris ini SEBELUMNYA SALAH.** Verifikasi langsung ke kode (`grep` tiap rule_id di `services/core-engine/app/`) membuktikan **F-F06, F-G04, F-G06–G14, F-C07-C10 SEMUA SUDAH terimplementasi, teruji, dan expose via endpoint** (`app/takeoff/{baja,atap,kusen,mep,mep_advanced,smkk}.py` + `/takeoff/baja|atap|kusen|mep|mep-advanced|smkk`, 11 test lulus `test_takeoff_baja_atap.py`/`test_takeoff_finishing_plus.py`/`test_takeoff_kusen_mep.py`). **UPDATE lanjutan (2026-07-05, sore, sesudah Fase X2/Task 1-4)**: gap bridging yang disebut baris ini SUDAH DITUTUP untuk dinding/atap(gording,trekstang,ikatan_angin)/kusen/MEP/kuda-kuda-baja/keramik_dinding/plafon/waterproofing — semua sekarang punya kategori terdeteksi (via AI-assist teks §Fase X2) DAN bridging ke core-engine (`bridging_{dinding,atap,kusen,mep,kuda_kuda,arsitektur_area}.py`). Sisa gap: 4 sub-domain `ArsitekturRequest` (`pondasi_batu`, `lantai`, `atap_miring`/penutup atap, `aanstamping`) — spek sudah ditulis (`docs/prompts/PAAX_CODEX_TASK_05_*.md`), **belum dijalankan** (Task 5, dihentikan sengaja oleh owner). Sisa §Z (confidence/QA §J/§K) belum diverifikasi ulang. | 🟢 rumus selesai, bridging non-struktur mayoritas 🟢 selesai (8 kategori), 4 sub-domain arsitektur sisa ⚪ spek ditulis belum dijalankan (Task 5) |
...
| Fase X2 (2026-07-05) | **AI-assist klasifikasi/binding berbasis teks** — LLM membaca teks YANG SUDAH DIEKSTRAK PyMuPDF (`Unclassified.raw`, bukan piksel) sbg fallback paralel saat rule-based gagal/ambigu; usulan wajib lolos validasi deterministik (angka harus match teks asli yang dikutip, enum/rentang tertutup) sebelum ditempel sbg field usulan — TIDAK PERNAH menimpa field asli/masuk input `core-engine` tanpa gerbang review manusia. Detail aturan: `CLAUDE.md` §1.1. Modul: `services/document-intelligence/app/perception/ai_assist/` (`client.py`, `dimension_assist.py`, `zone_assist.py`, `wall_assist.py`, `roof_frame_assist.py`, `kusen_assist.py`, `mep_assist.py`, `kuda_kuda_assist.py`, `arsitektur_area_assist.py`) + `bridging_{dinding,atap,kusen,mep,kuda_kuda,arsitektur_area}.py`. | 🟢 **8 slice selesai** (dimensi footplat, zona sheet, dinding, atap/gording-trekstang-ikatan_angin, kusen, MEP, kuda-kuda/profil baja, arsitektur area keramik/plafon/waterproofing). Dinding/atap/kusen/MEP awalnya diimplementasikan langsung oleh Claude, kuda-kuda & arsitektur-area oleh Codex (Task 1-4 lewat prompt file, pola dikoreksi ke Claude-desain/Codex-implementasi). **SUDAH DI-COMMIT & DI-MERGE** ke `main` (PR #40, dikonfirmasi ulang `gh pr view` 2026-07-05 sore — bukan lagi "belum di-commit"). document-intelligence naik 149 → 272 passed. `binding.py` (label→grid umum) SENGAJA tidak dikerjakan sbg AI-assist (murni geometris, tidak ada celah teks) — bukan gap tertunda, keputusan final. |
