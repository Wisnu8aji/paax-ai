...
| Fase 1 | Irisan pertama rumus takeoff: volume beton per bentuk (F-B01–B11) di `geometry/volume.py` + skeleton schema (Evidence diperkaya, ElementType/ElementInstance/WorkItem draft) | 🟡 sedang dikerjakan |
| Fase 2+ | F-C (bekisting), F-D (besi+BBS), parameter registry (§Z), F-F (tanah bank/gembur/padat), F-E/F-G (arsitektur/atap/kusen/MEP) | ⚪ roadmap, 1 sesi terpisah per irisan |
