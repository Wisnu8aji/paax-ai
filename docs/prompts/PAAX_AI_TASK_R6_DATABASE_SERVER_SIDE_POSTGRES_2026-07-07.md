# PROMPT SAYA — Task R6: Database Proyek Server-Side (Postgres/Cloud SQL)

> Ditulis Saya, 2026-07-07, reasoning tinggi. Bagian dari
> `docs/prompts/PAAX_SAYA_ROADMAP_10_TASKS_NON_UI_2026-07-07.md` (Task 6).
...
> (`docs: add task R6 prompt`) di AWAL branch Langkah 1 SEBELUM menulis
...
domain Saya). Kalau perubahan `lib/*.ts` butuh perubahan pemanggilan di
...
Saya", biarkan fungsi lama tetap ada berdampingan (backward-compatible)
sampai Saya yang migrasi pemanggilnya.
...
`.saya/launch.json` tidak ada konflik) ATAU tambahkan router baru ke
`services/core-engine` (PILIH SALAH SATU, pertimbangkan: core-engine
adalah "compute-only" per `SAYA.md` §3, jadi PISAH sebagai service baru
...
— dipakai NANTI oleh Saya saat wiring `apps/web` (tombol "impor data
lama" one-time), DAN
...
Tiap laporan: `report-remote/REPORT_TASKR6_LANGKAH<N>_<NAMA>_SAYA_<tanggal>.md`.
