# PROMPT CODEX — Fase J/K/L: Nyalakan "Generate RAB" dari Hasil Analisis Gambar

> Ditulis Claude, 2026-07-06. **Ini BUKAN prompt commit-saja** seperti prompt
> sebelumnya — Anda (Codex) diminta **mengimplementasikan kode** untuk fase
> ini, bukan cuma commit hasil kerja Claude. Owner (Wisnu) sudah membaca
...
1. **`CLAUDE.md`** (root repo) — terutama:
   ...
   - §9 **Pembagian tugas Claude vs Codex** dan **GERBANG REVIEW** (branch →
...
5. **`docs/prompts/PAAX_CODEX_PROMPT_COMMIT_GAMBAR_TEKNIK_SIPIL_2026-07-05.md`**
...
- Kalau ditemukan validator yang perlu logika baru (bukan cuma test) untuk
  ...
  **STOP, laporkan ke owner dulu** sebelum lanjut (lihat CLAUDE.md §9).
...
- **Auto-select HANYA kalau confidence tinggi** (mis. skor kandidat teratas
  ...
  tetap harus ada — CLAUDE.md §2: "Setiap fitur AI baru WAJIB punya
  fallback manual").
...
saja** — BUKAN perubahan permanen ke aturan gerbang review CLAUDE.md §9.
...
JANGAN merge sendiri — PR menunggu review owner + Claude dulu, sesuai
...
- **TIDAK mengubah rumus** di `services/core-engine/app/tkg/takeoff.py`
  (F-B/F-C/F-D) — kalau menemukan bug rumus, STOP dan laporkan (CLAUDE.md
  §9: perubahan yang menyentuh angka RAB perlu spek Claude dulu).
