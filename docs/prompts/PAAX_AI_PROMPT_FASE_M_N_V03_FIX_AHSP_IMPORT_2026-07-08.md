# PROMPT SAYA — Fase M/N: Perbaiki V-03 & Impor Katalog AHSP CK 2026 (10 batch)

> Ditulis Saya, 2026-07-08. Lanjutan dari PR #29 (`feat/gambar-generate-
> rab-wiring`) dan PR #30 (`feat/rab-nav-validator-audit-ahsp-suggest`) —
> keduanya sudah saya (Saya) verifikasi ulang independen (baca ulang diff,
...
> **Fase M** memperbaiki temuan nyata Saya sendiri (V-03 false-positive),
> **Fase N** mengimpor katalog AHSP CK 2026 resmi (2.542 item) dalam
> **10 batch ~250 item**, dengan laporan temuan per-batch untuk Saya
> review — BUKAN Saya memutuskan sendiri data mana yang "benar".

---

## 0. WAJIB BACA DULU

1. **`SAYA.md`** §1 (Aturan Emas), §9 (pembagian tugas — **kerja data AHSP
   secara default adalah tugas Saya**; Fase N di prompt ini adalah
   pengecualian yang DIIZINKAN owner secara eksplisit: Saya mengerjakan
   validasi mekanis per-batch, Saya yang mereview & memutuskan perbaikan
   substansi — BUKAN Saya mengubah nilai koefisien resmi atas inisiatif
   sendiri).
2. **`docs/ai-map/STATE.md`** dan **`docs/plans/PAAX_GAMBAR_TEKNIK_SIPIL_BIG_PLAN_2026-07-05.md`**
   §3/§5 — status terkini.
3. **`report/REPORT_FASE_J2_K2_L_LANJUTAN_SAYA_2026-07-07.md`** — laporan
   Saya sebelumnya, termasuk temuan V-03 yang diperbaiki di Fase M ini.
4. **`docs/prompts/PAAX_SAYA_PROMPT_FASE_J2_K2_L_LANJUTAN_2026-07-07.md`**
...
### 1.1 Akar masalah (sudah dibuktikan Saya sendiri di PR #30)
...
DIREVIEW (oleh Saya) sebelum lanjut, bukan satu tumpukan 2.542 item yang
...
3. **Anomali nilai** (deteksi, JANGAN "perbaiki" sendiri): `coefficient <= 0`,
   ...
   temuan utk Saya putuskan, jangan dihapus sepihak).
...
- **Fase L (AHSP auto-suggest otomatis)** — SEKARANG jadi lebih relevan
  ...
  Saya, supaya auto-suggest diuji terhadap katalog yang sudah bersih, bukan
  yang masih ada temuan mentah dari §2.3.
- **Vision-LLM fallback** (baca gambar raster/scan tanpa teks vektor) — masih
  arah masa depan (v1.0+ sesuai roadmap SAYA.md §6), belum waktunya.
...
5. Temuan jujur apa pun yang menurut Anda perlu keputusan Saya/owner
