# PROMPT SAYA — Fase 0: Golden anchor RAB nyata PLHUT + price book Surakarta (2026-07-03)
...
> Saya sudah **membangun & memverifikasi** fixture + 3 test golden anchor dari
...
> rumus/test/data.**
>
> Bukti terverifikasi Saya (jalankan ulang untuk pastikan):
> - **0a-1 HSP**: engine UMUM `app.rab.rab.compute_hsp()` reproduksi **32/32** HSP
>   profesional (baris F ALFA) via `(A+B+C)×(1+OP)` (SAYA.md §5).
...
## File yang SUDAH dibuat & diverifikasi Saya (JANGAN ubah isi)
...
**Test** (Saya tulis + verifikasi hijau):
...
- `docs/prompts/PAAX_SAYA_PROMPT_FASE0A_HSP_GOLDEN.md` (file ini)
...
git add docs/prompts/PAAX_SAYA_PROMPT_FASE0A_HSP_GOLDEN.md
git add docs/ai-map/STATE.md
```
JANGAN commit `.saya/`, `skills-lock.json`, `excel_extracted.txt`, `pdf_extracted.txt`.
...
Saya (112 resource, ada field `alfa_price_conflicts`) — cek isi sebelum `git add`.
