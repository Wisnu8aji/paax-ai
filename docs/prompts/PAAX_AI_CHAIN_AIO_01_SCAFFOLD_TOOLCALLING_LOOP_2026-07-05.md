# PROMPT SAYA — AI-Orchestrator Chain 01/2: Scaffold Service + Loop Tool-Calling Gemini + 2 Tool Proxy

> Ditulis Saya, 2026-07-05, reasoning tinggi, atas persetujuan EKSPLISIT
...
`MASTER_PLAN.md` §15.2 "mulai v0.8" TIDAK PERNAH DIBUAT**. Karena Saya
...
Saya BOLEH implementasikan penuh.

**Owner SUDAH menyetujui EKSPLISIT keputusan besar ini** ("semua saya
setujui") — TIDAK perlu tanya ulang. Tugas Saya sesi ini: implementasi
backend service baru SESUAI SPEK DI BAWAH INI (spek sudah dirancang
Saya, Saya TIDAK perlu membuat keputusan arsitektur baru, cukup ikuti).
...
1. **Ada scaffold LAMA yang TIDAK DIPAKAI**: `scripts/scaffolding/
   ...
   **Genkit** (`@genkit-ai/core`, `@genkit-ai/googleai`, `@genkit-ai/flow`)
   ...
   Genkit.** Keputusan desain (Saya, didokumentasikan eksplisit):
...
3. **Root `package.json`** SUDAH punya `@types/express` di
   `devDependencies` (temuan Saya, artefak dari usaha lama yang tidak
...
`apps/web` → `ai-orchestrator` adalah tugas Saya terpisah, BUKAN Saya).
...
- **Audit trail WAJIB**: setiap tool call (nama, args, ringkasan hasil)
  ...
  `SAYA.md` §1.1 yang sudah diterapkan di lapisan AI-assist
...
SENDIRI SEBELUM IMPLEMENTASI**: endpoint ini (dicek Saya,
...
konsisten Aturan Emas (`SAYA.md` §1: LLM boleh MEMILIH memanggil tool
...
Nama file baru: `report-remote/REPORT_AIO_CHAIN01_SCAFFOLD_TOOLCALLING_SAYA_<tanggal>.md`.
...
- **Commit HANYA oleh Saya**, boleh beberapa commit kecil (scaffold →
...
- **SETELAH task ini selesai DAN report (§7) sudah ditulis: LANGSUNG baca
  dan lanjutkan mengerjakan** `docs/prompts/PAAX_SAYA_CHAIN_AIO_02_
