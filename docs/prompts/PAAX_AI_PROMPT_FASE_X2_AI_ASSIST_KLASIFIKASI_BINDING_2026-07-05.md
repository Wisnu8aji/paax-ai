# PROMPT SAYA — Lapisan AI-Assist Klasifikasi/Binding, Slice Pertama: Dimensi Footplat (Fase X2)

> **STATUS (2026-07-05, update sesi yang sama): TIDAK dijalankan oleh
> Saya.** Owner mengubah instruksi setelah prompt ini ditulis: Saya
...
> (ditandai jelas dikerjakan Saya, BUKAN Saya) dan `docs/ai-map/STATE.md`
> §FASE X2. Jangan jalankan prompt ini via Saya lagi — sudah selesai.
>
> Ditulis Saya, 2026-07-05, atas instruksi owner. Konsep ini BARU
> disepakati owner-Saya sesi ini (bukan hasil analisis sepihak) dan sudah
> ditulis ke seluruh dokumen perencanaan sbg arah resmi project:
> `SAYA.md` §1.1, `AGENTS.md` §1.1, `docs/MASTER_PLAN.md` §6.2 & §12.1,
...
## 0. Konteks — kenapa fase ini ada (verifikasi sudah dilakukan Saya)

Sesi ini saya (Saya) memverifikasi `report-remote/REPORT_FASE_X1B_
PACKAGING_BINDING_SAYA_2026-07-05.md` langsung ke kode (bukan percaya
...
**Prinsip inti yang WAJIB dipegang** (detail lengkap `SAYA.md` §1.1):
...
- Klasifikasi AHSP (itu domain Tahap 3 pipeline, TS/`ai-orchestrator`,
  tidak berubah).
- `apps/web/**` — frontend/UI di luar cakupan Saya sesi ini (dikerjakan
  Saya terpisah kalau/ketika modul ini sudah siap dipakai UI).
...
- `client.py` — interface/protocol client (mis. `AiAssistClientProtocol`
  ...
  - **Cek dulu dependency yang SUDAH ada** di `services/document-
    ...
    catat alasannya eksplisit di laporan (`SAYA.md` §2: "jangan tambah
...
### 2.4 Schema — Pydantic + Zod mirror (WAJIB bersamaan, `SAYA.md` §1)
...
- **Saya**: seluruh §2-§3 (backend Python, schema Pydantic+Zod mirror,
  test). Commit HANYA oleh Saya, boleh jalan tanpa minta approval tiap
  langkah kecil (pola sesi sebelumnya).
- **TANPA `Co-Authored-By`/signature AI apa pun di commit manapun.**
- **Saya**: UI approval/review untuk `ai_suggestion` (kalau nanti
  dibangun) sepenuhnya di luar cakupan Saya — JANGAN sentuh `apps/web/**`
...
Nama file baru: `report-remote/REPORT_FASE_X2_AI_ASSIST_BINDING_SAYA_
