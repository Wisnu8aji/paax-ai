# PROMPT CODEX — AI-Orchestrator Chain 02/2: Tool query_rab/query_schedule/query_progress/query_materials + Integrasi Penuh

> Ditulis Claude, 2026-07-05. **Ini bagian 2 dari 2 file berantai** — WAJIB
> dikerjakan SETELAH `docs/prompts/PAAX_CODEX_CHAIN_AIO_01_SCAFFOLD_
...
> lalu BERHENTI. Wiring `apps/web` → `ai-orchestrator` adalah tugas Claude
...
bekerja adalah: **client (`apps/web`, di luar scope Codex) mengirim
...
adalah proyek migrasi besar terpisah, di luar scope). Tugas Codex: buat
...
Nama file baru: `report-remote/REPORT_AIO_CHAIN02_TOOLS_INTEGRASI_CODEX_<tanggal>.md`.
...
`apps/web`) menunggu Claude, bukan Codex.
...
- JANGAN mengarang data `query_progress`/`query_materials` dgn alasan
  ...
  pelanggaran Aturan Emas (`CLAUDE.md` §1) yg berlaku juga utk lapisan
