# PROMPT SAYA — Batch Brain v4.1: 10+ Irisan Engine (kerjakan semua, Saya review nanti)

> Owner mengizinkan Saya mengerjakan SEMUA task di bawah tanpa spek rinci per
...
> dan `app/tkg/takeoff.py` untuk pola besi. Saya akan REVIEW & PERBAIKI
...
- Branch baru per task atau per kelompok task terkait (nama bebas relevan),
  ...
  eksplisit (JANGAN `-A`/`.`), JANGAN stage `.saya/`/`skills-lock.json`.
...
`report/REPORT_BRAIN_BATCH_SAYA_2026-07-02.md` berisi task mana selesai/
