# PROMPT SAYA — Fase V+W: Normalisasi Kode Lintas-Halaman + Lapisan Item Pekerjaan (BOQ Grouping)

> Ditulis Saya, 2026-07-14, lanjutan langsung dari
...
> Saya langsung) tapi **belum di-commit** — prompt ini menggabungkan dua
...
> penuh** untuk tugas ini: Saya TIDAK perlu berhenti minta persetujuan
...
3. `SAYA.md` §1 (Aturan Emas) dan §9 (Pembagian Tugas Saya vs Saya) —
...
Owner memperjelas pembagian §9 SAYA.md/AGENTS.md khusus untuk task ini:

- **Saya (kamu) mengerjakan SEMUA hal berikut**: backend
...
- **Saya DILARANG KERAS menyentuh apa pun di `apps/web/src/components/**`
  atau `apps/web/src/app/**` (UI/frontend)** — itu murni domain Saya,
  ...
  - Pengecualian sempit: `packages/schemas/src/index.ts` (tipe Zod) BUKAN
    ...
    response baru.
- **Commit git HANYA boleh dilakukan oleh Saya** — Saya tidak akan
  commit apa pun untuk bagian frontend yang nanti dikerjakannya terpisah
  (Fase Y, sesi lain, model `saya-sonnet-5` reasoning tinggi — di luar
...
- Kalau di tengah jalan kamu menemukan keputusan yang menyentuh Aturan Emas
  (§1 SAYA.md) atau ambiguitas domain (mis. definisi kategori trade yang
  tidak jelas masuk seksi WBS mana) — **STOP, jangan menebak, catat di
  laporan sebagai pertanyaan terbuka untuk owner/Saya.**

---

## 2. STEP 0 — Commit backlog yang SUDAH diverifikasi (sebelum kerja baru)

Working tree saat ini di branch `fix/semarang-candidate-ranking-saya-direct`
berisi 3 kelompok pekerjaan yang SUDAH selesai & diverifikasi Saya langsung,
...
tugasmu untuk didiagnosis dari nol; itu tugas Saya/owner.
...
2. `feat(core-engine): AHSP auto-suggest untuk takeoff (Fase T)` — file di
   ...
   kecil yang SUDAH ditulis Saya, bukan kamu tulis ulang — kamu hanya
...
```powershell
git push origin fix/semarang-candidate-ranking-saya-direct
gh pr create --base main --head fix/semarang-candidate-ranking-saya-direct --draft `
...
bahan tampilan tabel RAB (dipakai UI-nya nanti oleh Saya di Fase Y — bukan
...
`fix/semarang-candidate-ranking-saya-direct` **setelah** commit di §2
...
`report/REPORT_FASE_V_W_SAYA_<tanggal-kamu-jalankan-ini>.md` (format nama
konsisten file report lain di folder ini, mis.
`REPORT_FASE_Q_R_TERAPKAN_HASIL_KEJAKSAAN_SEMARANG_SAYA_2026-07-11.md`).
...
- **JANGAN** menyentuh `apps/web/src/components/**` atau
  `apps/web/src/app/**` — domain Saya, bukan kamu, di prompt ini.
...
- **JANGAN** mulai Fase X (ekspansi rumus, terlalu besar, perlu prompt
  terpisah per-trade) atau Fase Y (UI 1-tombol, itu tugas Saya) di sesi
  ini — di luar scope, akan menyebabkan overscope yang melanggar prinsip
  vertical-slice (`SAYA.md`/`AGENTS.md` §2).
