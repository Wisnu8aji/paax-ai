# PROMPT CODEX — Perbaikan Temuan Fase V/W + Fase X-1: Bridging Galian Footplat

> Ditulis Claude, 2026-07-15. Ini lanjutan setelah verifikasi independen atas
> `report-remote/REPORT_FASE_V_W_CODEX_2026-07-05.md` (hasil eksekusi
> `docs/prompts/PAAX_CODEX_PROMPT_FASE_V_W_NORMALISASI_KODE_WORK_ITEMS_2026-
...
1. `docs/ai-map/STATE.md` — bagian "FASE V/W" (baru, ditulis Codex) untuk
   status terkini.
2. `report-remote/REPORT_FASE_V_W_CODEX_2026-07-05.md` — laporan yang
...
signature/attribution apa pun ke AI manapun (Claude/Codex/model lain).
...
menyertakan `Co-Authored-By: Claude Sonnet 5` — itu **tidak berlaku lagi
...
sendiri di `CLAUDE.md` §4 ("Deploy: Cloud Run (services)") — kalau
...
prinsip vertical-slice (`CLAUDE.md`/`AGENTS.md` §2), kerjakan **HANYA
...
- **JANGAN** menyentuh `apps/web/**` — sama seperti prompt sebelumnya, tetap
  domain Claude sepenuhnya.

---

## 3. Pembagian kerja, commit, dan gerbang review — SAMA seperti prompt sebelumnya

- **Codex (kamu)**: §1 (perbaikan) + §2 (Fase X-1) sepenuhnya — backend,
...
- **TANPA baris `Co-Authored-By`/signature apa pun di commit message** (lihat
  §0 — aturan baru).
- **Claude**: frontend (`apps/web/src/components/**`, `apps/web/src/app/**`)
  sepenuhnya di luar cakupan prompt ini, dikerjakan terpisah pakai
  `claude-sonnet-5` reasoning tinggi.
...
langsung bahwa `report-remote/` MEMANG SENGAJA diminta ke Codex, tujuannya
...
`REPORT_FASE_V_W_CODEX_2026-07-05.md`) — **selalu buat file BARU** dengan
...
Nama file: `report-remote/REPORT_FASE_X1_BRIDGING_GALIAN_CODEX_<tanggal-
eksekusi-nyata>.md` (contoh pola: `REPORT_FASE_V_W_CODEX_2026-07-05.md`
memakai tanggal Codex benar-benar menjalankan sesi, bukan tanggal prompt).
