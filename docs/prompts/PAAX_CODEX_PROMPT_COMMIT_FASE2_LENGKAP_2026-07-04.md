...
> `apps/web`) sudah dikerjakan LANGSUNG oleh Claude sesuai arahan owner
> ("Codex hanya bagian commit saja"), sudah diverifikasi lewat 339 test
...
Sesi ini (2026-07-04 malam, lanjutan) Claude membangun **seluruh pipeline
persepsi baca-gambar** (Fase 2 paket P1–P6 + rekonstruksi grid dari
geometri §3.1.1) langsung sebagai kode — bukan sebagai prompt Codex seperti
...
`report/REPORT_FASE2_PERSEPSI_LENGKAP_CLAUDE_2026-07-04.txt` — baca itu
...
docs/prompts/PAAX_CODEX_PROMPT_FASE2_P1_FONDASI_PERSEPSI.md
docs/prompts/PAAX_CODEX_PROMPT_FASE2_P2_LEKSIKON_GRAMMAR.md
docs/prompts/PAAX_CODEX_PROMPT_FASE2_P5_FIX_UI_PERSEPSI_REVIEW.md
docs/prompts/PAAX_CODEX_PROMPT_FASE2_P6_PADDLEOCR_RASTER.md
docs/prompts/PAAX_CODEX_PROMPT_COMMIT_FASE2_LENGKAP_2026-07-04.md
```
(4 file `PAAX_CODEX_PROMPT_FASE2_P*.md` berstatus "HISTORIS/SUPERSEDED" —
tetap di-commit sebagai jejak keputusan, JANGAN dihapus.)

### 2.7 Report
```
report/REPORT_CODEX_FULL_WORKLOG_UI_PREMIUM_DOC_INTEL_2026-07-04.md
report/REPORT_FASE2_PERSEPSI_LENGKAP_CLAUDE_2026-07-04.txt
```

### 2.8 Housekeeping — file terhapus (SUDAH ditandai terhapus SEBELUM sesi ini, bukan oleh Claude sesi ini)
```
...
.claude/launch.json   # sudah gitignored, jangan dipaksa masuk
```
...
- **JANGAN** mengubah logika apa pun di `app/perception/` atau
  ...
  Ini backend/frontend yang dikerjakan Claude atas keputusan sadar owner
  (bukan tugas normal Codex kali ini).
