...
import pytest
from paax_db.package_index import build_package_index_from_dem_pages, classify_page
...
if not PORTABLE_DB_PATH.is_file():
    PORTABLE_DB_PATH = Path("G:/PAAX-Data/db/portable.sqlite")


def test_package_index_88_pages_lossless_classification():
    """Verify package index includes 88/88 pages with 0 unassigned plans."""
    conn = sqlite3.connect(str(PORTABLE_DB_PATH))
    cur = conn.cursor()
    cur.execute("SELECT page_index, result FROM dem_pages ORDER BY page_index ASC")
    rows = cur.fetchall()
    assert len(rows) == 88, f"Expected 88 DEM pages, got {len(rows)}"

    dem_pages_data = [{"page_index": r[0], "result": json.loads(r[1]) if r[1] else {}} for r in rows]
    manifest = build_package_index_from_dem_pages(dem_pages_data, "PLHUT-SURAKARTA")

    assert manifest["total_pages"] == 88
    assert manifest["unassigned_count"] == 0, f"Expected 0 unassigned plans, got {manifest['unassigned_count']}"
...
def test_live_civil_work_items_pipeline_and_completeness():
    ...
    assert payload["generated_from"] == "project_graph_and_core_engine"

    summary = payload["summary"]
    assert summary["total_candidates"] >= 250
    assert summary["engine_verified_count"] == 8
    ...
    # Verify engine_verified items have valid engine receipts
    verified_items = [i for i in items if i["status"] == "engine_verified"]
    assert len(verified_items) == 8
