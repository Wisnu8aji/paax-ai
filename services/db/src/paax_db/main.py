...
def _portable_repo_root() -> Path:
    configured = getattr(app.state, "repo_root", None) or os.environ.get("PAAX_REPO_ROOT")
    if configured:
        return Path(configured).resolve()
    return Path(__file__).resolve().parents[4]


def _portable_project_manifest(project_id: str) -> dict[str, Any] | None:
    root = _portable_repo_root()
    manifest_path = Path(getattr(app.state, "portable_manifest_path", root / "fixtures" / "plhut" / "project-manifest.json"))
    ...
    try:
        ...
        return None
    return manifest if manifest.get("project_id") == project_id else None


def _portable_source_pdf(project_id: str) -> Path:
    manifest = _portable_project_manifest(project_id)
    if not manifest:
        raise HTTPException(status_code=404, detail="Portable source manifest not found")
    source = (_portable_repo_root() / manifest["source_document"]["path"]).resolve()
    if not source.is_file() or _portable_repo_root() not in source.parents:
        raise HTTPException(status_code=404, detail="Portable source PDF not found")
    actual_hash = hashlib.sha256(source.read_bytes()).hexdigest()
    if actual_hash != manifest["source_document"]["sha256"]:
        raise HTTPException(status_code=409, detail="Portable source PDF checksum mismatch")
    return source


@app.get(
    "/projects/{id}/source-document/manifest",
    dependencies=[Depends(RoleChecker(["estimator", "pm", "lapangan", "owner"]))],
)
async def get_project_source_manifest(id: str):
    manifest = _portable_project_manifest(id)
    if not manifest:
        raise HTTPException(status_code=404, detail="Source document manifest not found")
    return {
        "project_id": id,
        "file_name": manifest["source_document"]["path"],
        "sha256": manifest["source_document"]["sha256"],
        "page_count": manifest["source_document"]["page_count"],
        "pdf_url": f"/projects/{id}/source-document/pdf",
        "page_image_template": f"/projects/{id}/source-document/pages/{{page_index}}/image",
    }


@app.get(
    "/projects/{id}/source-document/pdf",
    dependencies=[Depends(RoleChecker(["estimator", "pm", "lapangan", "owner"]))],
)
async def get_project_source_pdf(id: str):
...
@app.get(
    "/projects/{id}/project-graph/civil-work-items",
    dependencies=[Depends(RoleChecker(["estimator", "pm", "lapangan", "owner"]))],
)
...
@app.post(
    "/projects/{id}/project-graph/engineering-context",
    dependencies=[Depends(RoleChecker(["estimator", "pm", "lapangan", "owner"]))],
)
async def get_project_engineering_context(id: str, request: dict[str, Any]):
    """Return a bounded, authoritative AI context for the bound project.

    Raw graph occurrence counts are intentionally not promoted to physical
    quantity. The projection only exposes verified Civil Work Items and explicit
    abstention rules.
    """
    query = str(request.get("query") or "").strip()
    if not query:
        raise HTTPException(status_code=400, detail="query is required")
    return build_engineering_context(id, _load_civil_work_items(id), query)


@app.get(
    "/projects/{id}/project-graph/civil-work-items/export.xlsx",
    dependencies=[Depends(RoleChecker(["estimator", "pm", "owner"]))],
)
async def export_civil_work_items(id: str):
    try:
        from openpyxl import Workbook
        from openpyxl.styles import Alignment, Font, PatternFill
    except ImportError as exc:
        raise HTTPException(status_code=503, detail="Excel export dependency is unavailable") from exc
    payload = _load_civil_work_items(id)
    workbook = Workbook()
    worksheet = workbook.active
    worksheet.title = "Perhitungan Backup"
    ...
    for item in handoff_items:
        ...
        worksheet.append([
            item.get("display_name"), item.get("location"), item.get("category"), item.get("unit"),
            item.get("dimensions_display"), item.get("count"), item.get("formula"), result_value,
            item.get("status"), source_text,
        ])
        result_cell = worksheet.cell(row=worksheet.max_row, column=8)
        unit = item.get("unit")
    ...
    for column, width in {"A":32,"B":16,"C":14,"D":10,"E":24,"F":10,"G":30,"H":16,"I":18,"J":48}.items():
        worksheet.column_dimensions[column].width = width
    worksheet.freeze_panes = "A2"
    worksheet.auto_filter.ref = worksheet.dimensions
    about = workbook.create_sheet("Sumber & Asumsi")
    about.append(["Project", id])
    ...
    stream.seek(0)
    filename = f"PAAX-{id}-Perhitungan-Backup.xlsx"
