...
def test_invalid_contextual_evidence_cases_are_rejected():
    ...
    for case in cases:
        ...
        with pytest.raises(ValidationError, match=r".*") as exc_info:
            if name == "malformed_sha256":
                RawEvidenceArtifact.model_validate(data)
            elif name in ("fact_no_evidence", "invalid_calculation_authority"):
                CanonicalFact.model_validate(data)
            elif name == "invalid_match_mode":
                PropagationScope.model_validate(data)
            elif name in ("approved_decision_no_selection", "approved_decision_no_decided_by"):
                ResolutionDecision.model_validate(data)
            else:
                pytest.fail(f"Unknown test case name: {name}")
